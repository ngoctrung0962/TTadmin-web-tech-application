...
export default function Order() {
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await ordersApi.update(
      order.orderId,
      order.user.username,
      formvalues
    );
    if (res !== undefined && res !== null) {
      setOrder(res);
      showNotification("success", "Update Order successful", "", "OK");
    }
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: (params) => {
        return (
          <div className="userListUser">{params.row.product.productId}</div>
        );
      },
    },
    {
    ...
    },
    {
      ...
      renderCell: (params) => {
        return (
          <div className="userListUser m-2">
            {" "}
            <img
              src={JSON.parse(params.row.product.image).image1}
              alt=""
      ...
      renderCell: (params) => {
        return (
          <div className="userListUser">
            {params.row.price.toLocaleString("vi", {
              currency: "VND",
            })}
          </div>
        );
    ...
    },
    {
      ...
      renderCell: (params) => {
        return (
          <div className="userListUser">
            {params.row.totalPrice.toLocaleString("vi", {
              currency: "VND",
            })}
          </div>
        );
  ...
  return (
    <div className="product">
        ...
        <div className="productTopRight">
          <div className="productInfoBottom">
            ...
            <div className="productInfoItem">
              ...
              <span className="productInfoValue">
                {order
                  ? order.totalPrices.toLocaleString("vi", {
                      currency: "VND",
                    }) + " VND"
                  : ""}
              </span>
            ...
            <div className="productInfoItem">
              ...
              <span className="productInfoValue">
                {order ? order.purchaseDate : ""}
              </span>
            </div>
            <div className="productInfoItem">
              <span className="productInfoKey">CouponID:</span>
              <span className="productInfoValue">
                {order.couponId ? order.couponId : "No coupon"}
              </span>
            </div>
            <div className="productInfoItem">
              <span className="productInfoKey">DiscountPrice:</span>
              <span className="productInfoValue">
                {order.discountPrice !== 0 ? `-${order.discountPrice}` : 0}
              </span>
            </div>
            <div className="productInfoItem">
              <span className="productInfoKey">DeliveryID:</span>
              <span className="productInfoValue">
                {order ? order.delivery?.deliveryId : ""}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="productBottom">
        ...
        <button onClick={handleSubmit} className="productAddButton">
          Edit
        </button>
      </div>
      <br></br>
